export type AudioFxOptions = {
  audioAssets?: Partial<
    Record<"drop1" | "drop2" | "merge" | "blast" | "success" | "bgm" | "start" | "restart" | "gameover", string>
  >;
};
...
export class AudioFx {
  ...
  private dropToggle = false;
  private clips = new Map<string, HTMLAudioElement>();
  private bgm?: HTMLAudioElement;

  constructor(options: AudioFxOptions = {}) {
    const entries = Object.entries(options.audioAssets ?? {}).filter(
  ...
  private playChord(freqs: number[], duration: number, volume: number): void {
